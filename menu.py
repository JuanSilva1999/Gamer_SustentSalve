    ...
    def move_cursor(self):  # Movimentação do Cursor (Setinha)
        if self.game.DOWN_KEY:  # Usando ceta pra baixo
            if self.state == 'Iniciar':
                ...
                self.cursor_rect.midtop = (self.startx + self.offset, self.starty)
                self.state = 'Iniciar'
        elif self.game.UP_KEY:  # Usando seta pra cima
            if self.state == 'Iniciar':
                self.cursor_rect.midtop = (self.exitx + self.offset, self.exity)
                self.state = 'Sair'
            elif self.state == 'Sair':
...
class OptionsMenu(Menu):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.game.draw_text('Como jogar', 40, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 - 120)
            ...
            self.game.draw_text("Atirar: Tecla F", 15, self.shotx, self.shoty)
...
class CreditsMenu(Menu):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.game.draw_text('CRIADOR DE SustentSalve', 20, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 4 - 20)
            self.game.draw_text('JUAN SILVA', 15, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + 10)
            self.game.draw_text("Voltar: Backspace", 10, self.mid_w - 200, self.mid_h + 190)
            self.game.draw_text("Avançar: Enter", 10, self.mid_w + 200, self.mid_h + 190)
            self.blit_screen()